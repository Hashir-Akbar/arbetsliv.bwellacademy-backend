...
var charJSPersonalDefaultOptionsHorizontalFactors = {} // NOBOX
...
/**
 * ctx.prototype
 * fillText option for cutting off text
 * @param text the string to cut
 * @param x x position
 * @param y y position
 * @param maxLength the max length for the text
 */
CanvasRenderingContext2D.prototype.fillTextCut = function(text, x, y, maxLength) {
    var textLength = this.measureText(text).width;
    if (textLength < maxLength)
        maxLength = textLength;
    var cutText = substring(0, maxLength - 3) + "...";
    this.fillText(cutText, x, y);
};
    ...
    for (i = 0; i < jsGraphAnnotate[ctx.ChartNewId]["length"]; i++) {
        if (jsGraphAnnotate[ctx.ChartNewId][i][0] == "ARC") {
            ...
            if (canvas_pos.x > jsGraphAnnotate[ctx.ChartNewId][i][1] && canvas_pos.x <= jsGraphAnnotate[ctx.ChartNewId][i][3] && canvas_pos.y < jsGraphAnnotate[ctx.ChartNewId][i][2] && canvas_pos.y > jsGraphAnnotate[ctx.ChartNewId][i][4]) {
                ...
                v20 = fmtChartJS(config, jsGraphAnnotate[ctx.ChartNewId][i][12], config.fmtV20); // v20=extra value NOBOX
                graphPosX = canvas_pos.x;
                graphPosY = canvas_pos.y;
                onData = true;
                if (action == "annotate") {
...
window.Chart = function(context) {
    ...
    this.HorizontalFactors = function(data, options) {
        chart.HorizontalFactors.defaults = {
            inGraphDataShow: false,
            inGraphDataPaddingX: 3,
            inGraphDataPaddingY: 0,
            inGraphDataTmpl: "<%=v3%>",
        ...
        // merge annotate defaults
        chart.HorizontalFactors.defaults = mergeChartConfig(chart.defaults.commonOptions, chart.HorizontalFactors.defaults);
        chart.HorizontalFactors.defaults = mergeChartConfig(chart.defaults.xyAxisCommonOptions, chart.HorizontalFactors.defaults);
        chart.HorizontalFactors.defaults = mergeChartConfig(chart.HorizontalFactors.defaults, charJSPersonalDefaultOptions);
        chart.HorizontalFactors.defaults = mergeChartConfig(chart.HorizontalFactors.defaults, charJSPersonalDefaultOptionsHorizontalFactors);
        var config = (options) ? mergeChartConfig(chart.HorizontalFactors.defaults, options) : chart.HorizontalFactors.defaults;
        return new HorizontalFactors(data, config, context);
    };
    chart.defaults = {};
    chart.defaults.commonOptions = {
        ...
        annotateBackgroundColor: '#000',//'rgba(0,0,0,0.8)',
        annotateFontSize: 12,
...
//                              jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", barOffset, xAxisPosY - yStart[j] + 1, barOffset + barWidth, barheight , lgtxt, lgtxt2, 1 * data.datasets[i].data[j], cumvalue[j], totvalue[j], i, j];
    ...
    var HorizontalStackedBar = function(data, config, ctx) {
        ...
        msr.availableHeight = msr.availableHeight - config.scaleTickSizeBottom - config.scaleTickSizeTop;
        msr.availableWidth = msr.availableWidth - config.scaleTickSizeLeft - config.scaleTickSizeRight - (config.paddingRight || 0); // NOBOX
        ...
        function drawBars(animPc) {
            ...
            var cumvalue = new Array();
            var totvalue = new Array();
            var actualvalue = new Array(); // NOBOX
            var datasetsLength = data.datasets.length; 
            for (var i = 0; i < datasetsLength; i++) {
                var dataLength = data.datasets[i].data.length;
                for (var j = 0; j < dataLength; j++) {
                    if (data.datasets[i].data[j] instanceof Array)
                    {
                        cumvalue[j] = new Array();
                        totvalue[j] = new Array();
                        actualvalue[j] = new Array();
                        var secondDataLength = data.datasets[i].data[j].length;
                        for (var k = 0; k < secondDataLength; k++) {
                            cumvalue[j][k] = 0;//data.datasets[i].data[j][k];
                            totvalue[j][k] = 0;
                            actualvalue[j][k] = 0;
                        };
                    }
            ...
            for (var i = 0; i < data.datasets.length; i++) {
                for (var j = 0; j < data.datasets[i].data.length; j++) {
                    if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
                        if (data.datasets[i].data[j] instanceof Array)
                        {
                            for (var k = 0; k < data.datasets[i].data[j].length; k++) {
                                totvalue[j][k] += 1 * data.datasets[i].data[j][k];
                            }
                        }
                        else
                        {
            ...
            for (var i = 0; i < data.datasets.length; i++) { // NOBOX
                for (var j = 0; j < data.datasets[i].realData.length; j++) {
                    if (!(typeof(data.datasets[i].realData[j]) == 'undefined')) {
                        if (data.datasets[i].data[j] instanceof Array)
                        {
                            for (var k = 0; k < data.datasets[i].data[j].length; k++) {
                                actualvalue[j][k] = data.datasets[i].realData[j][k];
                            }
                        }
                        else
                        {
                            actualvalue[j] = data.datasets[i].realData[j];
                        }
                    }
                }
            }
                    ...
                    if (!(typeof(data.datasets[i].data[j]) == 'undefined') && 1*data.datasets[i].data[j] != 0 ) {
                        ...
                        if(currentAnimPc !=0) {
                            ...
                            if (animPc >= 1) {
                                ...
                                if (1*data.datasets[i].data[j]<0) {
                                    jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", topBar, barOffset + barWidth, botBar, barOffset, lgtxt, lgtxt2, 1 * data.datasets[i].data[j], cumvalue[j], totvalue[j], i, j, actualvalue[j]]; // NOBOX
                                } else {
                                    jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", botBar, barOffset + barWidth, topBar, barOffset, lgtxt, lgtxt2, 1 * data.datasets[i].data[j], cumvalue[j], totvalue[j], i, j, actualvalue[j]]; // NOBOX
                                }
                            }
                        }
                        if (1*data.datasets[i].data[j]<0) {
                ...
                for (var i = 0; i < data.datasets.length; i++) {
                    ...
                    for (var j = 0; j < data.datasets[i].data.length; j++) {
                        ...
                        if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
                            ...
                            var dataVal = data.datasets[i].data[j];
                            var botBar=yAxisPosX + HorizontalCalculateOffset(botval, calculatedScale, valueHop);
                            var topBar=yAxisPosX + HorizontalCalculateOffset(topval, calculatedScale, valueHop);
                            ctx.save();
                            ctx.textAlign = config.inGraphDataAlign;
                            ...
                            var v20 = fmtChartJS(config, actualvalue[j], config.fmtV20); // NOBOX
                            var dispString = tmplbis(config.inGraphDataTmpl, {
                                config: config,
                                v1: fmtChartJS(config, lgtxt, config.fmtV1),
                                v2: fmtChartJS(config, lgtxt2, config.fmtV2),
                            ...
                            if (config.inGraphDataXPosition == 1) {
                                ...
                                xPos = topBar + config.inGraphDataPaddingX - 2; // NOBOX
                            }
                            if (config.inGraphDataYPosition == 1) {
                            ...
                            if (i == 0) { // NOBOX
                                ...
                                ctx.translate(width, yPos);
                                ctx.fillTextMultiLine(v20, 0, 0, ctx.textBaseline, config.inGraphDataFontSize);
                                ctx.restore();
                            }
                            if (1*data.datasets[i].data[j]<0) {
                                tempp[j]=tempp[j]+1*data.datasets[i].data[j] ;
        ...
        function drawLabels() {
            ...
            for (var j = 0; j < data.labels.length; j++) {
                ...
                if (config.yAxisRight) {
                    ...
                    if (data.selectionLabels && data.selectionLabels[j].length > 0) // NOBOX
                    {
                        ctx.textAlign = "left";
                        var len = data.selectionLabels[j].length;
                        for (var k = 0; k < len; k++)
                        {
                            // Vi behöver yAxisSpaceRight = 100 också
                            ctx.fillTextCut(
                                fmtChartJS(config, data.selectionLabels[j][k], config.fmtXLabel),
                                yAxisPosX,
                                ctx.textBaseLine,
                                100);
                        }
    ...
    var HorizontalFactors = function(data, config, ctx) { // NOBOX
        var maxSize, scaleHop, calculatedScale, labelHeight, scaleHeight, valueBounds, labelTemplateString, valueHop, widestXLabel, xAxisLength, yAxisPosX, xAxisPosY, barWidth, rotateLabels = 0,
            msr;
        if (config.reverseOrder) {
            data = reverseData(data);
        ...
        function drawBars(animPc) {
            ...
            for (var i = 0; i < data.datasets.length; i++) {
                for (var j = 0; j < data.datasets[i].data.length; j++)
                    if (!(typeof(data.datasets[i].data[j]) == 'undefined'))
                        totvalue[j] += 1 * data.datasets[i].data[j][0];
            }
            ctx.lineWidth = config.barStrokeWidth;
            for (var i = 0; i < data.datasets.length; i++) {
                if (animPc >= 1) {
                ...
                for (var j = 0; j < data.datasets[i].data.length; j++) {
                    ...
                    var barHeight = currentAnimPc * (calculateOffset(config.logarithmic, 1 * data.datasets[i].data[j][0], calculatedScale, valueHop) - zeroY) + (config.barStrokeWidth / 2);
                    ...
                    if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
                        ...
                        cumvalue[j] += 1 * data.datasets[i].data[j][0];
                        if (animPc >= 1) {
                            if (typeof(data.labels[j]) == "string") lgtxt2 = data.labels[j].trim();
                            else lgtxt2 = "";
                            t1 = yAxisPosX + zeroY;
                            t2 = yAxisPosX + calculateOffset(config.logarithmic, 1 * data.datasets[i].data[j][0], calculatedScale, valueHop) + (config.barStrokeWidth / 2)
                            if (t1 > t2) {
                                t3 = t1;
                                t1 = t2;
                                t2 = t3
                            }
                            jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", t1, barOffset + barWidth, t2, barOffset, lgtxt, lgtxt2, 1 * data.datasets[i].data[j][0], cumvalue[j], totvalue[j], i, j];
                        }
                    }
                }
            }
                    ...
                    for (var j = 0; j < data.datasets[i].data.length; j++) {
                        if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
                            ...
                            if (config.inGraphDataXPosition == 1) {
                                ...
                                xPos = yAxisPosX + (calculateOffset(config.logarithmic, 1 * data.datasets[i].data[j][0], calculatedScale, valueHop) + (config.barStrokeWidth / 2)) / 2 + config.inGraphDataPaddingX;
                            } else if (config.inGraphDataXPosition == 3) {
                                xPos = yAxisPosX + calculateOffset(config.logarithmic, 1 * data.datasets[i].data[j][0], calculatedScale, valueHop) + (config.barStrokeWidth / 2) + config.inGraphDataPaddingX;
                            }
                            ctx.translate(xPos, yPos);
                            cumvalue[j] += 1 * data.datasets[i].data[j][0];
                            var dispString = tmplbis(config.inGraphDataTmpl, {
                                ...
                                v1: fmtChartJS(config, lgtxt, config.fmtV1),
                                v2: fmtChartJS(config, lgtxt2, config.fmtV2),
                                v3: fmtChartJS(config, 1 * data.datasets[i].data[j][0], config.fmtV3),
                                v4: fmtChartJS(config, cumvalue[j], config.fmtV4),
                                v5: fmtChartJS(config, totvalue[j], config.fmtV5),
                                v6: roundToWithThousands(config, fmtChartJS(config, 100 * data.datasets[i].data[j][0] / totvalue[j], config.fmtV6), config.roundPct),
        ...
        function drawBars2(animPc) {
            ctx.lineWidth = config.barStrokeWidth;
            var tempp = new Array();
            var tempn = new Array();
            var cumvalue = new Array();
            var totvalue = new Array();
            var participants = new Array(); // NOBOX
            var datasetsLength = data.datasets.length; 
            for (var i = 0; i < datasetsLength; i++) {
                var dataLength = data.datasets[i].data.length;
                for (var j = 0; j < dataLength; j++) {
            ...
            for (var i = 0; i < data.datasets.length; i++) { // NOBOX
                for (var j = 0; j < data.datasets[i].participants.length; j++) {
                    if (!(typeof(data.datasets[i].participants[j]) == 'undefined')) {
                        participants[j] = data.datasets[i].participants[j];
                    }
                }
            }
            for (var i = 0; i < data.datasets.length; i++) {
                ...
                for (var j = 0; j < data.datasets[i].data.length; j++) {
                    ...
                    tempn[j] = new Array();
                    for (var k = 0; k < 2; k++) {
                        var currentAnimPc = animationCorrection(animPc, data, config, i, k, 1).animVal;
                        if (currentAnimPc > 1) currentAnimPc = currentAnimPc - 1;
                        if (i == 0) {
                            tempp[j][k]=0;
                            tempn[j][k]=0;
                        ...
                        var barOffset = xAxisPosY + config.barValueSpacing - scaleHop * (j + 1);
                        if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
                            if (1*data.datasets[i].data[j][k]<0) {
                                var botval=tempp[j][k];
                                var topval=tempp[j][k]+1*data.datasets[i].data[j][k];
                            } else {
                                var botval=tempn[j][k];
                                var topval=tempn[j][k]+1*data.datasets[i].data[j][k];
                            }
                            if(config.animationByDataset) {
                                var botBar=yAxisPosX + HorizontalCalculateOffset(botval , calculatedScale, valueHop);
                                var topBar=yAxisPosX + HorizontalCalculateOffset(topval , calculatedScale, valueHop);
                            ...
                            if (k == 0) {
                                ctx.fillStyle = "#6fac84";
                                ctx.strokeStyle = "#6fac84";
                            }
                            else {
                                ctx.fillStyle = "#f34f98";
                                ctx.strokeStyle = "#f34f98";
                            }
                            if(currentAnimPc !=0) {
                                ctx.beginPath();
                                ctx.moveTo(botBar, barOffset);
                                ...
                                ctx.closePath();
                                ctx.fill();
                                cumvalue[j][k] += 1 * data.datasets[i].data[j][k];
                                if (animPc >= 1) {
                                    if (typeof(data.labels[j]) == "string") lgtxt2 = data.labels[j].trim();
                                    else lgtxt2 = "";
                                    if (1*data.datasets[i].data[j][k]<0) {
                                        jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", topBar, barOffset + barWidth, botBar, barOffset, lgtxt, lgtxt2, 1 * data.datasets[i].data[j][k], cumvalue[j][k], totvalue[j][k], i, j, participants[j]]; // NOBOX
                                    } else {
                                        jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", botBar, barOffset + barWidth, topBar, barOffset, lgtxt, lgtxt2, 1 * data.datasets[i].data[j][k], cumvalue[j][k], totvalue[j][k], i, j, participants[j]]; // NOBOX
                                    }
                                }
                            }
                            if (1*data.datasets[i].data[j][k]<0) {
                                tempp[j][k]=tempp[j][k]+1*data.datasets[i].data[j][k] ;
                            } else {
                                tempn[j][k]=tempn[j][k]+1*data.datasets[i].data[j][k] ;
                            }
                        }
                    }
                }
                    ...
                    for (var j = 0; j < data.datasets[i].data.length; j++) {
                        ...
                        if (!(typeof(data.datasets[i].data[j]) == 'undefined')) {
                            for (var k = 0; k < 2; k++) {
                                ...
                                var dataVal = data.datasets[i].data[j][k];
                                var botBar=yAxisPosX + HorizontalCalculateOffset(botval, calculatedScale, valueHop);
                                var topBar=yAxisPosX + HorizontalCalculateOffset(topval, calculatedScale, valueHop);
                                ctx.save();
                                ctx.textAlign = config.inGraphDataAlign;
                                ...
                                var barOffset = xAxisPosY + config.barValueSpacing - scaleHop * (j + 1);
                                cumvalue[j][k] += data.datasets[i].data[j][k];
                                var v20 = fmtChartJS(config, participants[j], config.fmtV20); // NOBOX
                                var dispString = tmplbis(config.inGraphDataTmpl, {
                                    config: config,
                                    v1: fmtChartJS(config, lgtxt, config.fmtV1),
                                    v2: fmtChartJS(config, lgtxt2, config.fmtV2),
                                    v3: fmtChartJS(config, 1 * data.datasets[i].data[j], config.fmtV3),
                                    v4: fmtChartJS(config, cumvalue[j][k], config.fmtV4),
                                    v5: fmtChartJS(config, totvalue[j][k], config.fmtV5),
                                    v6: roundToWithThousands(config, fmtChartJS(config, 100 * data.datasets[i].data[j][k] / totvalue[j][k], config.fmtV6), config.roundPct),
                                    v7: fmtChartJS(config, yAxisPosX, config.fmtV7),
                                    v8: fmtChartJS(config, barOffset + barWidth, config.fmtV8),
                                    v9: fmtChartJS(config, yAxisPosX + HorizontalCalculateOffset(data.datasets[i].data[j][k], calculatedScale, valueHop) + (config.barStrokeWidth / 2), config.fmtV9),
                                    v10: fmtChartJS(config, barOffset, config.fmtV10),
                                    v11: fmtChartJS(config, i, config.fmtV11),
                                    v12: fmtChartJS(config, j, config.fmtV12),
                                    v20: v20, // NOBOX
            ...
            if(msr.legendMsr.dispLegend)drawLegend(msr.legendMsr,data,config,ctx,"HorizontalFactors");
        };
    ...
    function animationLoop(config, drawScale, drawData, ctx, clrx, clry, clrwidth, clrheight, midPosX, midPosY, borderX, borderY, data) {
        ...
        function animateFrame() {
            ...
            if (typeof config.onAnimateFrame == "function") { // NOBOX
                config.onAnimateFrame(ctx);
            }
        };
